...
function search() {
  ...
  if (j < 0) {
    ...
    if (k == size) {
      ...
      console.log(
        `New Queen is placed in position [${k - 1}, ${queens[k - 1]}]`
      );
    }
  }
}
...
function findPosition(k) {
  ...
  for (let j = start; j < size; j++) if (isValid(k, j)) return j;
  return -1;
}

// is the queen position follows the constraints?
